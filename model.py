...
class CartPendulum(BaseSystem):
    ...
    def draw_frame(self, ax, i, x, u, alpha=1.0, x_pred=None):
        ...
        ax.add_patch(
            plt.Circle((x[0, i] + math.sin(x[1, i]), -math.cos(x[1, i])), self.p.l / 8, color="blue", alpha=alpha)
        )
        ax.plot(
    ...
    def constraints(self, x, u):
        h1 = x[2]
        h2 = x[1] - cs.pi
        return cs.vertcat(h1, h2) # Return both constraints
...
class Pendubot(BaseSystem):
    ...
    def __init__(self):
        ...
        self.a1, self.a2, self.a3, self.a4, self.a5 = (
            self.p.I1 + self.p.m1 * self.p.d1**2 + self.p.I2 + self.p.m2 * (self.p.l1**2 + self.p.d2**2),
            self.p.m2 * self.p.l1 * self.p.d2,
            self.p.I2 + self.p.m2 * self.p.d2**2,
            self.g * (self.p.m1 * self.p.d1 + self.p.m2 * self.p.l2),
            self.g * self.p.m2 * self.p.d2,
        )
        ...
        self.line1 = (
            lambda q, u: -self.p.fr1 * q[2]
            - self.a4 * cs.sin(q[0])
            - self.a5 * cs.sin(q[0] + q[1])
            - self.a2 * cs.sin(q[1]) * q[3] * (q[3] + 2 * q[2])
            + u
        )
        self.line2 = (
            lambda q, u: -self.p.fr2 * q[3] - self.a5 * cs.sin(q[0] + q[1]) - self.a2 * cs.sin(q[1]) * q[2] ** 2
        )
        self.f1 = lambda q, u: (self.m22(q, u) * self.line1(q, u) - self.m12(q, u) * self.line2(q, u)) / (
            self.m11(q, u) * self.m22(q, u) - self.m12(q, u) ** 2
        )
        self.f2 = lambda q, u: (-self.m12(q, u) * self.line1(q, u) + self.m11(q, u) * self.line2(q, u)) / (
            self.m11(q, u) * self.m22(q, u) - self.m12(q, u) ** 2
        )
        self.f = lambda x, u: cs.vertcat(x[2:4], self.f1(x, u), self.f2(x, u))
    ...
    def draw_frame(self, ax, i, x, u, alpha=1.0, x_pred=None):
        ...
        p2 = (
            self.p.l1 * math.sin(x[0, i]) + self.p.l2 * math.sin(x[0, i] + x[1, i]),
            -self.p.l1 * math.cos(x[0, i]) - self.p.l2 * math.cos(x[0, i] + x[1, i]),
        )
